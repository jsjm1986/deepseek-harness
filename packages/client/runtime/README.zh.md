...
`sessions/prepare-create` 是创建根会话时使用的 Client waterfall。`SessionRuntime.create()` 会在 RPC 分发前让调用方提供的 `SessionCreateOptions` 经过该事件，并且只发送最终返回的选项；监听器可以增加插件自有字段或拒绝创建。每个监听器都必须先调用 `next()`，再应用自己的贡献，让独立插件能够组合。fork 与 subagent 不使用该事件。
