...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。每个浏览器都通过 Host settings API 读取 `ui-onboarding.welcomeNoticeVersion`，且只有明确点击「继续」才会记录当前版本。可写的 settings 投影通过 `settings.mutate` 持久化该版本；只读投影则仅在当前 client 进程内记录，绝不尝试 Host 写入，连接重置后仍保留确认，整页重载后声明会再次出现。
