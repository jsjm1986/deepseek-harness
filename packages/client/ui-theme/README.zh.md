...
`src/styles/` 下有六张样式表，全部由 web 壳的 `base.css` 导入：`base.css`、`metrics.css`、`design-platform.css`、`scrollbar.css`、`gradient-shadow-text.css` 与 `shiki.css`。`metrics.css` 声明与主题无关的布局度量 token（`--dsw-space-*`、`--dsw-radius-*`、`--dsw-safe-*`、`--dsw-touch-target`）。`scrollbar.css` 是 `--dsw-alias-scrollbar-*` token 的唯一消费方，必须排在声明这些 token 的 `design-platform.css` 之后。
