...
这些原子组件承载 [web-styling](../../../docs/web-styling.md) 规定的触控基线：compact 视口下 Modal 变为底部面板、Menu 子菜单改为内联展开；粗指针设备上菜单行与关闭控件放大到共享触控目标、Input 文字保持 16px（避免 iOS 聚焦缩放）、Tooltip 保持隐藏（锚点保留各自的 aria-label）。
