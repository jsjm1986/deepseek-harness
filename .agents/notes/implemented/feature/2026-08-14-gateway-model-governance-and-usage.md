...
The gateway owns one SQLite-backed governance plane keyed by exact `(provider, model)` routes. The catalog stores enabled state, role defaults for `admin` and `user`, per-user allow/deny/inherit exceptions, append-only effective-time prices for input/output/cache token classes, and role/user monthly token and company-cost quotas. Per-user quota fields distinguish inherit, unlimited, and a non-negative custom value. Natural months are computed in the configurable IANA `HGW_USAGE_TIME_ZONE` (default `Asia/Shanghai`). Quotas create durable idempotent 80% and 100% alerts; they never block execution.

Policy is projected atomically to each instance as mode-`0600` `$DSH_HOME/model-governance.json`, including a stable bearer credential for the loopback intake. A policy edit rewrites the projection and restarts only an already-running affected instance. The mandatory tree-external `dsh-model-governance` bundle is mounted independently of the optional directory guard, so `HGW_GUARD_PATCH=off` cannot disable model authorization or accounting. Its built JavaScript has no non-Node runtime imports: it publishes the typed `ctx.modelAccess` face as a plain object instead of loading another Cordis copy from its real filesystem location.
...
Every terminal call outcome is first committed as a UUID-named JSON file by same-directory rename in a per-instance outbox. A background pump posts files to the bearer-authenticated loopback intake and removes them only after success. SQLite uses the UUID as the primary key, so retries are idempotent. Price selection uses the event occurrence time; personal calls retain estimated cost but contribute zero company cost. Missing provider usage becomes an explicit `missing-usage` outcome rather than fabricated token counts. Denials are also written to the gateway audit log.

The admin SPA exposes Models and Usage pages for catalog, role defaults, user exceptions, prices, quotas, and natural-month summaries. Authenticated users read only their own summary through `/account/api/usage`; a client plugin contributes durable threshold alerts to `shell.overlay` without recomputing quota policy in the browser.

## Testing

Gateway tests cover policy precedence and global disable, price calculation and personal-cost exclusion, UUID deduplication, threshold idempotency, malformed intake rejection, time-zone month boundaries, quota inheritance/unlimited/custom semantics, bearer intake authentication and denial audit, policy projection permissions/token reuse, and mandatory governance when directory guard is disabled. Plugin tests cover fail-closed policy parsing, final-stream denial and usage capture, attribution conflict, and outbox retry/removal. Host tests cover catalog filtering plus selection/prompt refusal. Adapter tests pin the non-secret source label. Type builds cover host, client, admin UI, model-access, the alert bundle, and the tree-external plugin.
...
The deployment gains one enforceable model policy across all model call paths, historical price-correct monthly accounting, conservative company-cost attribution, durable alerts, and admin/user visibility without storing model content or secrets. Policy changes to live instances require a restart, quota enforcement remains advisory, unknown credential sources count as company cost, and the gateway now depends on a mandatory built tree-external plugin plus a private loopback intake port. Production packaging must copy that plugin with its checked-in `lib/` artifacts; disabling directory confinement does not disable governance.
