...
Conversation controller 持有仅限浏览器的草稿标识与元数据，主机持有 durable 标识。粘贴、拖放和附件选择器会让图片 MIME 类型继续走既有图片路径，其余文件全部走流式文档客户端。输入栏显示带上传进度、就绪／失败状态、重试和移除操作的文档 rail。上传完成前不能提交；提示失败会恢复文档标识与草稿文本，提示成功则释放浏览器元数据而保留 durable 文件。移除草稿会中止进行中的上传，并尽力删除已落盘文件；服务 teardown 会中止剩余上传。
