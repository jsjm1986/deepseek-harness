...
**欢迎声明复用既有持久化字段。** 完整文案与版本由 `onboarding-copy.ts` 持有。客户端通过既有 settings API 比较和写入 `ui-onboarding.welcomeNoticeVersion`，且只有点击「继续」才确认当前版本。Host 特权方法栅栏仍要求回环 `Host` 头；网关改写后公网页才能持久化该确认。不改变 Host schema、API Proxy 允许列表或持久化实现。
