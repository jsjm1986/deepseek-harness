...
**The welcome notice reuses the existing durable field.** Its exact copy and version live in `onboarding-copy.ts`. Clients compare and write `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Continue acknowledges the current version. The Host privileged-method fence still requires a loopback `Host` header; a gateway rewrite is what makes a public page persist the acknowledgement. No Host schema, API-proxy allowlist, or persistence implementation changes.
