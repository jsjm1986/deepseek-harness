...
Every instance carries the guard uniformly (fail-loud when the configured patch file is missing — an unguarded start is worse than a refused one, and `HGW_GUARD_PATCH=off` is the explicit opt-out). The deny reason is model-visible and pinned by a snapshot spec (`tests/guard.snapshot.spec.ts`), so rewording is a conscious diff. The plugin owns no authentication and no kernel boundary; it is one layer of the defense stack, honest about the bash gap. The gateway's home-layer copy means a plugin patch update reaches instances on their next restart, not live.

## Testing

Pure-logic vitest suites cover grant loading/classification, path canonicalization (symlink and `..` escapes), per-tool read/write mapping, fail-closed malformed arguments, and the pinned deny wording. The gateway acceptance script (`gateway/scripts/accept-phase1.sh`, 23 checks green on macOS) verifies the mount end to end: home patch layer written, package linked, grants file present before start, and the composed tree showing the `directory-guard` row with `danger-full-access` absent (`--dump-config`). Model-driven end-to-end denial (an agent attempting an out-of-grant write) remains in the manual bucket documented in the plugin README — it needs a real API key.
