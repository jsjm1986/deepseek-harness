...
真实 PostgreSQL 17.6 容器覆盖直到版本 3 的 migration、公共 ID 保留与分配、企业隔离、完整 SQLite 控制面导入、认证与会话撤销、用户/项目管理、个人/项目实例状态、共享对话持久化与 ACL、审计查询、模型策略、用户/项目额度继承、精确微单位货币换算、幂等用量写入和阈值告警。SQLite 单元测试与组装后的 HTTP/proxy 测试继续覆盖共享路由和实例生命周期行为。生产验收检查本机与公网健康、鉴权后的管理端资源和 API、个人/项目代理、双成员共享与私密对话行为、`ro` 拒绝、私有用量 intake、不再产生 SQLite 写入，以及切换后的 PostgreSQL 恢复校验。
