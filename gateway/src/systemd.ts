...
export function renderUserUnit(
  ...
  for (const grant of grants) {
    ...
    if (!runtimeOwned && !projectPath) throw new Error(`grant is outside managed roots: ${grant.path}`)
  ...
  const masks = [...new Set([opts.usersRoot, opts.projectRuntimesRoot, ...opts.projectPathRoots])]
