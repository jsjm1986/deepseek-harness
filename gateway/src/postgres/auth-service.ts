...
export class PostgresAuthService {
  ...
  Promise<{ token: string; user: UserRow } | 'invalid' | 'locked'> {
    ...
    const result = await this.context.pool.query<PostgresAuthUser>(`SELECT u.public_id::text,u.username::text,
      ...
      WHERE u.organization_id=$1 AND u.username=$2 AND u.deleted_at IS NULL`, [this.context.organizationId, username])
    const row = result.rows[0]
    const accepted = row !== undefined && row.deleted_at === null && row.status === 'active' && row.membership_status === 'active'
    ...
    const created = await transaction(this.context.pool, async (client) => {
      ...
      const user = await client.query<{ id: string }>(
        `SELECT u.id FROM harness.users u
          JOIN harness.memberships m ON m.organization_id=u.organization_id AND m.user_id=u.id
          WHERE u.organization_id=$1 AND u.public_id=$2 AND u.deleted_at IS NULL
            AND u.status='active' AND m.status='active' FOR UPDATE OF u,m`,
