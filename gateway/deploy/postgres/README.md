...
This directory runs the first PostgreSQL data-plane baseline for the Gateway. It uses one PostgreSQL 17 database: ordinary columns for control data, JSONB for append-only conversation events, and host paths for large local files. It does not add MongoDB, Redis, object storage, or a second service tier.

> **Storage selection:** the production Gateway still opens `gateway.sqlite`. These files, migrations, repositories, and importer are an independently verified migration baseline. Setting a production database URL does not switch the current Gateway entry point.
