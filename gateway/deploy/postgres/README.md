...
It drops only the `harness` schema in the supplied test database. Never point it at production. Coverage includes immutable migrations through version 6, rejection of an unknown migration ledger, organization isolation, arbitrary string session IDs, full JSON round trips including NUL strings, contiguous sequence enforcement, concurrent batch idempotency, nested tool-result search, repeatable SQLite import including project invitations and deletion timestamps, root-inherited collaboration ACLs, contribution projections, interaction races, shared project runtime allocation from a configured empty-node port base, project credentials/quotas/usage, logical user deletion and active-row login checks, and the live authentication, user, project, node-instance, audit, and model-governance services.
