...
4. Copy `deploy/harness-gateway.service` to `/etc/systemd/system/`, adjust `HGW_PUBLIC_ORIGINS`, `HGW_MODEL_GOVERNANCE_PACKAGE`, and other paths if they differ, then `systemctl daemon-reload && systemctl enable --now harness-gateway`.
...
- Gateway behavior: `bash scripts/accept-phase1.sh` (any host, 36 checks, no API key).
