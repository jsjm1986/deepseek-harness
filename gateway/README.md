...
The Users page can logically delete an account after confirmation; deletion revokes access and stops its runtime while retaining history, and the API protects the last administrator and the current administrator. The conversation composer also exposes an attachment picker for images and documents in addition to paste and drag-and-drop; documents retain the existing streamed upload and visible-file semantics.
